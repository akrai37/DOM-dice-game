...
var score, roundscore, activePlayer, gameplaying; //All these are global variables. 'gameplaying' is a binary variable defined as 'true' in init() fn. Unless the button satisfies the 'true' condition, its data won't be executed.
init();
...
document.querySelector('.btn-roll').addEventListener('click', function(){ //'addEventListener()' contains an event and a fn (fn is the eventlistener) as arguments. 'event' is the name of the work it does(here 'click') and the 'EventListener' is the function that defines it.
    ...
    if(gameplaying){
            //1. Random number for both tthe dices
            var dice1= Math.floor(Math.random()* 6) + 1;
            var dice2= Math.floor(Math.random()* 6) + 1;

           //2. displaying the result which includes displaying the images of both dices.
            var dice1DOM= document.querySelector('#dice-1'); // '#dice-1' is an 'id' which is representing an 'image'(refer html source file). We are calling the image and storing it in a different variable(it doesnt matter what image it is right now because we are gonna change it according to the dice number) .
            dice1DOM.style.display='block'; //making the display of 'diceDOM' as 'block'(making the whole spac occupied by '.dice' class as just a block)
            dice1DOM.src= 'dice-'+ dice1 +'.png'; //we have changed or manipulated the source of '.dice' letting it equal to the name of one of the dice images present in the folder.

            var dice2DOM= document.querySelector('#dice-2'); //'#dice-2' is a 'id' which is representing an 'image'(refer html source file). We are calling the image and storing it in a different variable(it doesnt matter what image it is right now because we are gonna change it according to the dice number) .
            dice2DOM.style.display='block'; //making the display of 'diceDOM' as 'block'(making the whole spac occupied by '.dice' class as just a block)
            dice2DOM.src= 'dice-'+ dice2 +'.png';

            //3.Update the round score IF any of the number is not 1.
            if(dice1!==1&&dice2!==1){
                  //add score
                  roundscore+= (dice1+ dice2);
                  if((dice1+ dice2)==12){
                        //making the main score of active player 0 because of '6' on both the dices.
